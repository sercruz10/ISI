...
{
    // NOTE: You can use the "Rename" command on the "Refactor" menu to change the interface name "IIHumanControlServicos" in both code and config file together.
    [ServiceContract]
    public interface IIHumanControlServicos
    {
        ...
        [OperationContract]
        [WebInvoke(Method = "POST", RequestFormat = WebMessageFormat.Json, ResponseFormat = WebMessageFormat.Json, UriTemplate = "Colaborador")]
        string Nome(Picagem colaborador);

        [OperationContract]
        [WebInvoke(Method = "GET", RequestFormat = WebMessageFormat.Json, ResponseFormat = WebMessageFormat.Json, UriTemplate = "Names")]
        List<NomeNumero> Names();
