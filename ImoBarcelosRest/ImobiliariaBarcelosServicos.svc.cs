...
{
    // NOTE: You can use the "Rename" command on the "Refactor" menu to change the class name "ImobiliariaBarcelosServicos" in code, svc and config file together.
    // NOTE: In order to launch WCF Test Client for testing this service, please select ImobiliariaBarcelosServicos.svc or ImobiliariaBarcelosServicos.svc.cs at the Solution Explorer and start debugging.
    [WebService(Namespace = "http://localhost:30463/ImobiliariaBarcelosServicos.svc/", Description = "ImobiliariaBarcelos", Name = "IMOBARCELOS")]
    public class ImobiliariaBarcelosServicos : IImobiliariaBarcelosServicos
    {
        ...
        public List<Pessoa> Pessoas()
        {
            ...
            try
            {
                ...
                try
                {
                    while (reader.Read())
                    {
                        pessoas.Add(new Pessoa() { IdPessoa = Convert.ToInt32(reader["IdPessoa"]), Nome = Convert.ToString(reader["Nome"]), Morada = Convert.ToString(reader["Morada"]), DataNascimento = Convert.ToString(reader["DataNascimento"]), Telefone = Convert.ToString(reader["Telefone"]), CartaoCidado = Convert.ToString(reader["CartaoCidado"]), Email = Convert.ToString(reader["Email"]), NIF = Convert.ToString(reader["NIF"]), Sexo_IdSexo = Convert.ToInt32(reader["Sexo_IdSexo"]) });
        ...
        Habitacao IImobiliariaBarcelosServicos.HabitacaoId(string id)
        {
